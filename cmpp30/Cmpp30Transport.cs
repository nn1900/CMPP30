...
namespace Reefoo.CMPP30
{
  internal class Cmpp30Transport : IDisposable
  {
    ...
    private readonly ILog log = LogManager.GetLogger(typeof (Cmpp30Transport));
    ...
    public void Connect()
    {
      lock (_lock)
      {
        ...
        _socket.BeginConnect(_config.GatewayIp, _config.GatewayPort, delegate(IAsyncResult result)
        {
          ...
          {
            log.Error("Unable to connect, error: ", ex);
          }
          connectEndEvent.Set();
        }, _socket);
        ...
        log.ErrorFormat(
          "Fail to connect to remote host {0}:{1}", 
          _config.GatewayIp, 
          _config.GatewayPort
          );
        Disconnect();
        ...
        try
        {
          lock (_lock)
          {
            ...
            if (_socket.Poll(50000, SelectMode.SelectRead))
            {
              ...
              {
                log.ErrorFormat("error receive: {0}", e);
                // in case socket.Receive got exception
                Disconnect();
                continue;
              }
    ...
    private void _DataReady()
    {
      ...
      else
      {
        ...
        var commandId = _state.Header.Value.CommandId;
        var commandType = MessageTypes[commandId];
        log.InfoFormat("Received message {0}: {1}", commandId, commandType.Name);
        _state.Size = CmppConstants.HeaderSize;
        _state.PackageType = typeof (CmppHead);
        _state.Header = null;
      }
